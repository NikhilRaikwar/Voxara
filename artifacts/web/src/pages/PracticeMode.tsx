...
export default function PracticeMode() {
  ...
  const { currentTrack, targetLanguage, addAttempt } = useSession();
  const { t } = useI18n();

  if (!currentTrack || lineIdxParam === null) {
    setLocation("/");
    return null;
  }
  ...
  const handleNext = () => {
    if (sessionData && lineIndex < sessionData.lines.length - 1) {
      clearAudio();
      setGradingResult(null);
      // Navigate to the next line; the change in lineIndex recomputes the model
      // audio URL and the mount effect auto-plays it.
      setLocation(`/practice?line=${lineIndex + 1}`);
    } else {
      setLocation("/recap");
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col container mx-auto max-w-3xl px-4 py-8 items-center justify-center min-h-[80vh]">
      <button
        ...
        className="absolute top-24 left-4 md:left-8 flex items-center gap-2 text-muted-foreground hover:text-foreground"
      >
        <ArrowLeft className="w-4 h-4" />{" "}
        <span>{t("practice.backToListen")}</span>
      </button>
      ...
      <div className="w-full max-w-2xl text-center space-y-12">
        ...
        <div className="flex justify-center pb-8">
          ...
          >
            {isModelPlaying ? (
              <span className="animate-pulse">{t("practice.playing")}</span>
            ) : (
              <>
                <Play className="w-5 h-5" /> {t("practice.playModel")}
              </>
            )}
          </Button>
        </div>
        ...
        {gradingResult && (
          <div className="bg-card border border-border/50 rounded-3xl p-8 shadow-sm animate-in slide-in-from-bottom-8 fade-in">
            <div className="flex items-center justify-between mb-8 pb-6 border-b border-border/50">
              ...
              <div
                className={`text-3xl font-bold ${gradingResult.scorePercent > 80 ? "text-green-600 dark:text-green-500" : gradingResult.scorePercent > 50 ? "text-amber-500" : "text-destructive"}`}
              >
            ...
            <div className="flex flex-wrap gap-x-2 gap-y-4 justify-center text-xl font-medium mb-10">
              {gradingResult.words.map((gw: any, i: number) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`${isMatch ? "text-green-600 dark:text-green-500" : "text-destructive underline decoration-wavy decoration-destructive/50"}`}
                    >
                      {gw.expected}
                    </span>
                    {!isMatch && (
